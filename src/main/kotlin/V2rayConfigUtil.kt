...
object V2rayConfigUtil {
     ...
     * 生成v2ray的客户端配置文件
     */
    fun getV2rayConfig(config : ServerConfig): Result {
        try {

            val outbound = config.getProxyOutbound() ?: return Result(false, "")
            val result = getV2rayNonCustomConfig(outbound)
//            println(result.content)
    ...
    // ------------- GFW knocker------------------
    fun readTextFromLocalFile(filename:String) : String {
        try {
            val rawtext = File(filename).inputStream().bufferedReader().use { it.readText() }
            return rawtext
        }catch (e : Exception){
            println(e.toString())
        }
    ...
    fun readOneLineFromLocalFile(filename:String) : String {
        try {
            val rawtext = File(filename).inputStream().bufferedReader().readLine()
            return rawtext
        }catch (e : Exception){
            println(e.toString())
            return ""
    ...
    fun writeTextToLocalFile(filename:String , rawtext:String):Boolean {
        try {
            val bw = File(filename).outputStream().bufferedWriter()
            bw.write(rawtext)
            bw.flush()
            return true
        }catch (e : Exception){
     ...
     * 生成v2ray的客户端配置文件
     */
    private fun getV2rayNonCustomConfig(outbound: V2rayConfig.OutboundBean): Result {
        val result = Result(false, "")
        //取得默认配置
        val assets = readTextFromLocalFile("v2ray_config.json")
        ...
        v2rayConfig.log.loglevel = AppConfig.PREF_LOGLEVEL?: "warning"
...
//
            val socksPort = Utils.parseInt(AppConfig.PORT_SOCKS , 10808)
            val httpPort = Utils.parseInt(AppConfig.PORT_HTTP , 10809)
